...
public class CPV2Item extends ShieldItem {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (!world.isClient && (entity instanceof PlayerEntity player)) {
            if (player.getEquippedStack(EquipmentSlot.OFFHAND).isOf(FriendsChatOriginsItems.CPV2)) {
                player.addStatusEffect(new StatusEffectInstance(FriendsChatOriginsEffects.CPV2, 2, 0, false, true));
            }
            if (player.getEquippedStack(EquipmentSlot.MAINHAND).isOf(FriendsChatOriginsItems.CPV2)) {
                player.addStatusEffect(new StatusEffectInstance(FriendsChatOriginsEffects.CPV2, 2, 0, false, true));
            }
        }
    }
